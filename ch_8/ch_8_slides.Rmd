...
\only<2->{\begin{textblock}{6}(1,3.7)
...
\only<3>{\begin{textblock}{7}(8.5,4)
\begin{alertblock}{}
...
\begin{block}{Random walk forecasts}
  \centerline{${y}{T+h}{T} = y_T$}
\end{block}\pause

\begin{block}{Average forecasts}
  \centerline{$\displaystyle{y}{T+h}{T} = \frac1T\sum_{t=1}^T y_t$}
...
\centerline{${y}{T+1}{T} = \alpha y_T + \alpha(1-\alpha) y_{T-1} + \alpha(1-\alpha)^2 y_{T-2}+ \cdots$,}\mbox{}\hfill where $0 \le \alpha \le1$.
\end{block}\pause
...
# anim_save('alpha_anim', path = './gif/')
```

![Algerian exports ETS alpha animation](../gif/alpha_anim.gif)

## Simple Exponential Smoothing

\begin{block}{Component form}\vspace*{-0.4cm}
\begin{align*}
\text{Forecast equation}&&{y}{t+h}{t} &= \ell_{t}\\
\text{Smoothing equation}&&\ell_{t} &= \alpha y_{t} + (1 - \alpha)\ell_{t-1}
\end{align*}
\end{block}\vspace*{-0.2cm}

* $\ell_t$ is the level (or the smoothed value) of the series at time t.
* ${y}{t+1}{t} = \alpha y_t + (1-\alpha) {y}{t}{t-1}$
...
$\displaystyle{y}{T+1}{T}=\sum_{j=0}^{T-1} \alpha(1-\alpha)^j y_{T-j}+(1-\alpha)^T \ell_{0}$
\end{block}
...
\end{align*}
\end{block}\pause
Forecast error: $e_t = y_t - {y}{t}{t-1} = y_t - \ell_{t-1}$.\pause
...
## ETS(M,N,N): SES with multiplicative errors.
  ...
  * Specify relative errors  $\varepsilon_t=\frac{y_t-{y}{t}{t-1}}{{y}{t}{t-1}}\sim \text{NID}(0,\sigma^2)$
  * Substituting ${y}{t}{t-1}=\ell_{t-1}$ gives:
    * $y_t = \ell_{t-1}+\ell_{t-1}\varepsilon_t$
    * $e_t = y_t - {y}{t}{t-1} = \ell_{t-1}\varepsilon_t$
...
\begin{align*}
\text{Forecast }&& {y}{t+h}{t} &= \ell_{t} + hb_{t} \\
...
\pause\vspace*{-0.2cm}
  ...
  * $\ell_t$ level: weighted average between $y_t$ and one-step ahead forecast for time $t$, $(\ell_{t-1} + b_{t-1}={y}{t}{t-1})$
...
# anim_save('beta_anim.gif', path = './gif/')
```

![Australian population trend beta animation](../gif/beta_anim.gif)
...
\begin{align*}
{y}{t+h}{t} &= \ell_{t} + (\phi+\phi^2 + \dots + \phi^{h})b_{t} \\
...
\pause
  ...
  * As $h\rightarrow\infty$, ${y}{T+h}{T}\rightarrow \ell_T+\phi b_T/(1-\phi)$.
...
\begin{block}{Component form}\vspace*{-0.4cm}
\begin{align*}
{y}{t+h}{t} &= \ell_{t} + hb _{t} + s_{t+h-m(k+1)} \\
...
# anim_save('gamma_anim.gif', path = './gif/')
```

![Medicare Australia cost of vaccine scripts seasonality gamma animation](../gif/gamma_anim.gif)
...
\begin{block}{Component form}\vspace*{-0.3cm}
    \begin{align*}
        {y}{t+h}{t} &= (\ell_{t} + hb_{t})s_{t+h-m(k+1)} \\
...
\begin{block}{}\vspace*{-0.4cm}
\begin{align*}
{y}{t+h}{t} &= [\ell_{t} + (\phi+\phi^2 + \dots + \phi^{h})b_{t}]s_{t+h-m(k+1)} \\
